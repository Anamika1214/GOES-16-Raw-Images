...
def processing(pathData, pathScripts):
    ...
    Start = (pathData[pathData.find(INPE_Band_ID + "_")+4:pathData.find(".nc")])
    ...
    bmap.readshapefile(pathScripts + '/Shapefiles/BRA_adm1','BRA_adm1',linewidth=0.50,color='cyan')
    bmap.readshapefile(pathScripts + '/Shapefiles/ne_10m_admin_0_countries','ne_10m_admin_0_countries',linewidth=0.50,color='cyan')
    ...
    cpt = loadCPT(pathScripts + '/Colortables/Square Root Visible Enhancement.cpt')
    ...
    # Save the result
    # print(pathScripts + '/Output/' + pathData[12:18] + '/INPE_G16_CH' + str(Band) + '_' + Start + '.png')
    plt.savefig(pathScripts + '/Output/Band06/INPE_G16_CH' + str(Band) + '_' + Start + '.png', dpi=DPI, bbox_inches='tight', pad_inches=0)

# Path to the GOES-R simulated image file
path = '/GEONETCast/Band06/'
pathScripts = "/Scripts/GEONETCast"

# Create output folder if it does not exist
# "Band01, Band02, ..."
try:
    os.mkdir(pathScripts + "/Output/Band06/")
...
for file in dirs:
    try:
        processing(path + file, pathScripts)
    except OSError:
        print("Erro Arquivo")
